...
/*   Updated: 2024/10/22 10:48:03 by aaghzal          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../ft_printf.h"

void	flag_handler(const char *p_format, int *p_i,
			int flag_data[FLAG_DATA_SIZE], va_list args)
{
	if (p_format[(*p_i)] == '-')
		...
		flag_data[WIDTH] = va_arg(args, int);
	...
	else if (p_format[(*p_i)] == '.')
	{
		(*p_i) += 1;
		if (p_format[(*p_i)] == '*')
			flag_data[PRECISION] = va_arg(args, int);
		else if (ft_isdigit(p_format[(*p_i)]))
